...
import { augmentChai, nowTime, jumpBlock, ABOOK_ADDRESS, nowBlock } from "../../helpers/utils";
...
describe("Registry.sol", function () {
  ...
  describe("Transfer owner of registry", function () {
    it("#transferOwnership: Can't transfer ownership to zero address", async function () {
      ...
      await expect(registry.transferOwnership(ZeroAddress)).to.be.revertedWith("Zero address");
    });
  ...
  describe("Check register function", function () {
    ...
    it("#register: Successfully register system contract", async function () {
      ...
      await expect(registry.register("MockUpgradeable", mockProxy.target, activeBlockForMockUpgradeable)).to.emit(
        registry,
        "Registered",
      );

      // Not active yet
      expect(await registry.getActiveAddr("MockUpgradeable")).to.equal(ZeroAddress);

      await jumpBlock(activeBlockForMockUpgradeable);

      // Now it's active
      expect(await registry.getActiveAddr("MockUpgradeable")).to.equal(mockProxy.target);
    ...
    it("#register: Register zero address means deprecation", async function () {
      ...
      // It overwrites the previous contract since it is not active yet
      await expect(registry.register("MockUpgradeable", ZeroAddress, activeBlockForMockUpgradeable * 2)).to.emit(
        registry,
        "Registered",
      );
    ...
    describe("Pre-deployed system contracts", function () {
      ...
      it("Deprecate pre-deployed contract: AddressBook", async function () {
        ...
        await expect(registry.register("AddressBook", ZeroAddress, activeBlockForMockUpgradeable * 2)).to.emit(
          registry,
          "Registered",
        );
        ...
        // Now it's considered as deprecated
        expect(await registry.getActiveAddr("AddressBook")).to.equal(ZeroAddress);
      });
      it("Deprecate pre-deployed contract: StakingTracker", async function () {
        ...
        await expect(registry.register("StakingTracker", ZeroAddress, activeBlockForMockUpgradeable * 2)).to.emit(
          registry,
          "Registered",
        );
        ...
        // Now it's considered as deprecated
        expect(await registry.getActiveAddr("StakingTracker")).to.equal(ZeroAddress);
      });
    });
    ...
    describe("Contract replacement test", function () {
      ...
      it("Update mock upgradeable system contracts by proxy", async function () {
        ...
        const mockProxyWithV2 = await upgrades.upgradeProxy(mockProxy.target, MockUpgradeableSystemContractV2);
