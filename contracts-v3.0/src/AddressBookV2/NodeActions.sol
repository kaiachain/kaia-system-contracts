...
///         and system-triggered (processSystemTransition).
abstract contract NodeActions is AddressBookV2Base {
    ...
    function updateVoterAddress(address nodeId, address newVoterAddress) external onlyManager(nodeId) {
        ...
        address tracker = IRegistry(REGISTRY_ADDRESS).getActiveAddr("StakingTracker");
        if (tracker != address(0)) IStakingTracker(tracker).refreshVoter(nodeId);
    ...
    function readyCandidate(address nodeId) external onlyNodeId(nodeId) {
        ...
        if (nodeId.balance < MIN_NODE_BALANCE) revert InsufficientNodeBalance();
    ...
    function processSystemTransition(
        ...
        for (uint256 i; i < len; ++i) {
            _transition(nodeIds[i], newStates[i], timeoutAts[i]);
        }
        if (_isEpochBlock()) {
