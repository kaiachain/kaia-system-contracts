...
abstract contract AddressBookLegacy {
    ...
    // uint256 public constant MAX_ADMIN = 50; // removed
    // uint256 public constant MAX_PENDING_REQUEST = 100; // removed
    ...
    // uint256 public constant ONE_WEEK = 1 weeks; // removed
    // uint256 public constant TWO_WEEKS = 2 weeks; // removed
    ...
    function getAllAddress() external view returns (uint8[] memory typeList, address[] memory addressList) {
        if (!isActivated) {
            ...
            (address[] memory nodeIds, address[] memory staking, address[] memory rewards, address kif, address kef) =
                _getAllAddressData();
            uint256 cnNodeCnt = nodeIds.length;
            typeList = new uint8[](cnNodeCnt * 3 + 2);
            addressList = new address[](cnNodeCnt * 3 + 2);
            uint256 j;
            for (uint256 i; i < cnNodeCnt; ++i) {
                typeList[j] = CN_NODE_ID_TYPE;
                addressList[j++] = nodeIds[i];
                typeList[j] = CN_STAKING_ADDRESS_TYPE;
                addressList[j++] = staking[i];
                typeList[j] = CN_REWARD_ADDRESS_TYPE;
                addressList[j++] = rewards[i];
            }
            typeList[j] = POC_CONTRACT_TYPE;
            addressList[j++] = kif;
            typeList[j] = KIR_CONTRACT_TYPE;
            addressList[j] = kef;
        }
    }
