...
contract AddressBookV2 is NodeActions, AddressBookLegacy {
    ...
    function initialize() public initializer {
        ...
        // Register addresses and set initial active validators
        uint256 len = d.nodeIds.length;
        for (uint256 i; i < len; ++i) {
            address nid = d.nodeIds[i];
            NodeInfo memory info = d.infos[i];
            $.usedAddresses[nid] = true;
            $.usedAddresses[info.stakingContract] = true;
            $.usedAddresses[info.rewardAddress] = true;
    ...
    function updateSuspender(address newSuspender) external onlyOwner {
        emit AddressConfigUpdated(ADDR_CFG_SUSPENDER, ABv2ConfigLib.SUSPENDER.updateAddress(newSuspender), newSuspender);
    }

    /// @inheritdoc IAddressBookV2
    function updateConfigurator(address newConfigurator) external onlyOwner {
        emit AddressConfigUpdated(
            ADDR_CFG_CONFIGURATOR, ABv2ConfigLib.CONFIGURATOR.updateAddress(newConfigurator), newConfigurator
        );
    }
    ...
    function updatePauseTimeout(uint256 newPauseTimeout) external onlyConfigurator {
        emit UintConfigUpdated(
            UINT_CFG_PAUSE_TIMEOUT, ABv2ConfigLib.PAUSE_TIMEOUT.updateUint(newPauseTimeout), newPauseTimeout
        );
    }
    ...
    function updateIdleTimeout(uint256 newIdleTimeout) external onlyConfigurator {
        emit UintConfigUpdated(
            UINT_CFG_IDLE_TIMEOUT, ABv2ConfigLib.IDLE_TIMEOUT.updateUint(newIdleTimeout), newIdleTimeout
        );
    }
    ...
    function updateMaxNodeCount(uint256 newMaxNodeCount) external onlyConfigurator {
        emit UintConfigUpdated(
            UINT_CFG_MAX_NODE_COUNT, ABv2ConfigLib.MAX_NODE_COUNT.updateUint(newMaxNodeCount), newMaxNodeCount
        );
    }
    ...
    function updatePfsThreshold(uint256 newPfsThreshold) external onlyConfigurator {
        emit UintConfigUpdated(
            UINT_CFG_PFS_THRESHOLD, ABv2ConfigLib.PFS_THRESHOLD.updateUint(newPfsThreshold), newPfsThreshold
        );
    }
    ...
    function updateCfsThreshold(uint256 newCfsThreshold) external onlyConfigurator {
        emit UintConfigUpdated(
            UINT_CFG_CFS_THRESHOLD, ABv2ConfigLib.CFS_THRESHOLD.updateUint(newCfsThreshold), newCfsThreshold
        );
    }

    /// @inheritdoc IAddressBookV2
    function updateKefAddress(address newKefAddress) external onlyConfigurator {
        emit AddressConfigUpdated(ADDR_CFG_KEF, ABv2ConfigLib.KEF_ADDRESS.updateAddress(newKefAddress), newKefAddress);
    }

    /// @inheritdoc IAddressBookV2
    function updateKifAddress(address newKifAddress) external onlyConfigurator {
        emit AddressConfigUpdated(ADDR_CFG_KIF, ABv2ConfigLib.KIF_ADDRESS.updateAddress(newKifAddress), newKifAddress);
    }

    /// @inheritdoc IAddressBookV2
    function updateKpfAddress(address newKpfAddress) external onlyConfigurator {
        emit AddressConfigUpdated(ADDR_CFG_KPF, ABv2ConfigLib.KPF_ADDRESS.updateAddress(newKpfAddress), newKpfAddress);
    }
    ...
    /// @inheritdoc IAddressBookV2
    function getAllGovernanceInfo() external view override returns (GovernanceInfo[] memory infos) {
        ABv2Storage storage $ = _getStorage();
        uint256 len = $.allNodes.length();
        infos = new GovernanceInfo[](len);
        for (uint256 i; i < len; ++i) {
            address nid = $.allNodes.at(i);
            NodeInfo storage info = $.nodeInfo[nid];
            infos[i] = GovernanceInfo(nid, info.stakingContract, info.voterAddress, info.gcId);
        }
    }
    ...
    /// @inheritdoc AddressBookLegacy
    function _getNodeData(address nodeId)
        internal
        view
        override
        returns (address stakingContract, address rewardAddress, bool exists)
    {
        ABv2Storage storage $ = _getStorage();
