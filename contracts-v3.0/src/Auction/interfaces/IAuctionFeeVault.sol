...
interface IAuctionFeeVault {
    ...
    /// @notice Emitted on owner-driven batch registration and on permissionless single
    ///         register/update calls. The `caller` field distinguishes these for indexers.
    event RewardAddressRegistered(address indexed nodeId, address indexed reward, address indexed caller);
    ...
    function registerRewardAddresses(address[] memory nodeIds, address[] memory rewardAddrs) external;
