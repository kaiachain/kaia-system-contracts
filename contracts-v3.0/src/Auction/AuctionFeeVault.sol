...
import "../AddressBookV2/interfaces/IAddressBookV2.sol";
...
contract AuctionFeeVault is IAuctionFeeVault, Ownable, AuctionError {
    ...
    /// @dev AddressBookV2 lives at the well-known system precompile address (0x...0400).
    ...
    function takeBid(address searcher) external payable override {
        ...
        if (validatorPayback > 0) {
            ...
            if (rewardAddr != address(0)) {
                ...
                ///      Need to restrict the gas limit for deterministic gas calculation.
                (bool success, ) = payable(rewardAddr).call{value: validatorPayback, gas: 5000}("");
                if (!success) {
                    validatorPayback = 0;
    ...
    /// @dev Owner-only batch registration, used for bootstrapping.
    ...
    function registerRewardAddresses(
        ...
        for (uint256 i = 0; i < nodeIds.length; i++) {
            ...
            emit RewardAddressRegistered(nodeId, rewardAddr, msg.sender);
        }
    }

    /// @dev v3.0 change: replaces v2.1's CnStaking-admin check with an AddressBookV2 manager check.
    /// @param nodeId The CN node ID registered as a validator in AddressBookV2.
