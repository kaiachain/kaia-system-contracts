...
contract AuctionEntryPoint is IAuctionEntryPoint, AuctionError, Nonces, EIP712, Ownable {
    ...
    function call(AuctionTx calldata auctionTx) external override onlyProposer depositVaultNotEmpty {
    ...
    function changeDepositVault(address _depositVault) external override onlyOwner notNull(_depositVault) {
    ...
    function changeAuctioneer(address _auctioneer) external override onlyOwner notNull(_auctioneer) {
    ...
    function _checkAndTakeBid(address searcher, uint256 bidAmount, uint256 callGasLimit) internal returns (bool) {
    ...
    function _payGas(address searcher, uint256 initialGas) internal returns (bool) {
        uint256 _gasUsed = _getMaximumGas(initialGas - gasleft() + gasBufferUnmeasured);
        /// @dev The tx.gasprice will be multiplied by the gasUsed in the depositVault
        return depositVault.takeGas(searcher, _gasUsed);
    }

    function _getMaximumGas(uint256 executionGas) internal view returns (uint256) {
    ...
    function _verifyInputIntegrity(AuctionTx calldata auctionTx) internal view returns (bool) {
        ...
        /// 5. Check if the auctioneer signature is valid
        ...
        /// 6. Check if the searcher signature is valid
    ...
    function _getAuctionTxHash(AuctionTx calldata auctionTx) internal pure returns (bytes32 auctionTxHash) {
    ...
    function getAuctionTxHash(AuctionTx calldata auctionTx) external pure override returns (bytes32) {
        return _getAuctionTxHash(auctionTx);
    }

    function getNoncesAndDeposits(
        address[] memory searchers
    ) external view override returns (uint256[] memory nonces_, uint256[] memory deposits_) {
