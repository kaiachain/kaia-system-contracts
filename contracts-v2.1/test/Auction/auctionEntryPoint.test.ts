...
import { fillTypeDataArgs, getMiner, nowBlock, toPeb } from "../../helpers/utils";
import { Signer } from "ethers";
import { toBeArray, hexlify, parseEther, randomBytes, ZeroAddress } from "ethers";
...
async function generateAuctionTx(
  ...
  const signature = await sender.signTypedData(typeData.domain, typeData.types, typeData.message);

  const aucSignature = await auctioneer.signMessage(toBeArray(signature));
...
describe("AuctionEntryPoint", () => {
  describe("Check initialize", () => {
    ...
    it("Check initialize", async () => {
      const { auctionEntryPoint, deployer, auctionDepositVault } = await auctionTestFixture();

      expect(await auctionEntryPoint.depositVault()).to.equal(await auctionDepositVault.getAddress());
      expect(await auctionEntryPoint.auctioneer()).to.equal(deployer.address);
    });
  });
  describe("Check configuration", () => {
    it("Only owner", async () => {
      const { auctionEntryPoint, user1 } = await loadFixture(auctionTestFixture);

      await expect(auctionEntryPoint.connect(user1).changeAuctioneer(user1.address)).to.be.revertedWithCustomError(
        auctionEntryPoint,
        "OwnableUnauthorizedAccount",
      );
      await expect(auctionEntryPoint.connect(user1).changeDepositVault(user1.address)).to.be.revertedWithCustomError(
        auctionEntryPoint,
        "OwnableUnauthorizedAccount",
      );
      await expect(
        auctionEntryPoint.connect(user1).changeGasParameters(16, 40, 21_000, 180_000, 30_000),
      ).to.be.revertedWithCustomError(auctionEntryPoint, "OwnableUnauthorizedAccount");
    });
    it("Not null address", async () => {
      const { auctionEntryPoint, deployer } = await loadFixture(auctionTestFixture);

      await expect(auctionEntryPoint.connect(deployer).changeAuctioneer(ZeroAddress)).to.be.revertedWithCustomError(
        auctionEntryPoint,
        "ZeroAddress",
      );
      await expect(auctionEntryPoint.connect(deployer).changeDepositVault(ZeroAddress)).to.be.revertedWithCustomError(
        auctionEntryPoint,
        "ZeroAddress",
      );
    });
    it("Change auctioneer", async () => {
      const { auctionEntryPoint, deployer, user1 } = await loadFixture(auctionTestFixture);

      await expect(auctionEntryPoint.connect(deployer).changeAuctioneer(user1.address))
    ...
    it("Change deposit vault", async () => {
      const { auctionEntryPoint, auctionDepositVault, deployer, user1 } = await loadFixture(auctionTestFixture);

      await expect(auctionEntryPoint.connect(deployer).changeDepositVault(user1.address))
        .to.emit(auctionEntryPoint, "ChangeDepositVault")
        .withArgs(auctionDepositVault.getAddress(), user1.address);
      expect(await auctionEntryPoint.depositVault()).to.equal(user1.address);
    });
    it("Change gas parameters", async () => {
      const { auctionEntryPoint, deployer } = await loadFixture(auctionTestFixture);

      await expect(auctionEntryPoint.connect(deployer).changeGasParameters(100, 100, 100, 100, 100))
  ...
  describe("Check call", () => {
    ...
    beforeEach(async () => {
      ...
      const miner = await ethers.provider.getBlock("latest").then((block) => block?.miner);
      if (!miner) {
    ...
    it("#call: Only proposer", async () => {
      ...
      const { auctionTx } = await generateAuctionTx(
        await auctionEntryPoint.getAddress(),
        user1,
        deployer,
        await testReceiver.getAddress(),
        "0x",
        await nowBlock(),
      );

      await expect(auctionEntryPoint.connect(user1).call(auctionTx)).to.be.revertedWithCustomError(
        auctionEntryPoint,
        "OnlyProposer",
      );
    });
    it("#call: reverted if invalid signature", async () => {
      ...
      await expect(auctionEntryPoint.connect(coinbase).call(auctionTx)).to.be.reverted;
    ...
    it("#call: reverted if invalid auctioneer signature", async () => {
      ...
      auctionTx.auctioneerSig = "0x" + Buffer.from(randomBytes(65)).toString("hex");
    ...
    it("#call: reverted if failed to verify the caller integrity", async () => {
      ...
      await expect(auctionEntryPoint.connect(coinbase).call(auctionTx2)).to.be.reverted;
      ...
      await expect(auctionEntryPoint.connect(coinbase).call(auctionTx3)).to.be.reverted;
    });
    it("#call: should take bid and gas reimbursement if failed to call the target", async () => {
      const { auctionEntryPoint, auctionDepositVault, auctionFeeVault, user1, deployer, testReceiver } = fixture;
      ...
      const { auctionTx } = await generateAuctionTx(
        ...
        user1,
        deployer,
        await auctionFeeVault.getAddress(), // target isn't test receiver
        calldata,
        (await nowBlock()) + 1,
        10_000_000,
        toPeb(10n),
      ...
      const spentGas = user1Deposit - BigInt(toPeb(10n)) - (await auctionDepositVault.depositBalances(user1.address));
