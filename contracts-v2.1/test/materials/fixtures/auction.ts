...
import { registerContract, setRegistryOwner, specialContractsFixture } from "../../../helpers/utils";
...
export const auctionTestFixture = async () => {
  ...
  const auctionFeeVault = await new AuctionFeeVault__factory(deployer).deploy(deployer.address, 0, 1000);
  const auctionDepositVault = await new AuctionDepositVault__factory(deployer).deploy(
    deployer.address,
    await auctionFeeVault.getAddress(),
  );
  const auctionEntryPoint = await new AuctionEntryPoint__factory(deployer).deploy(
    deployer.address,
    await auctionDepositVault.getAddress(),
    deployer.address,
  );
  ...
  await registerContract(registry, "AuctionEntryPoint", await auctionEntryPoint.getAddress());
