...
import { MockCLRegistry__factory } from "../../typechain-types";
...
/**
 * @dev This unit & scenario test is same as stakingTracker.test.ts
 * except that it uses StakingTrackerV2.sol.
 *
 * This test is for checking compatibility between StakingTrackerV1.sol and StakingTrackerV2.sol.
 */
describe("StakingTrackerV2.sol", function () {
  ...
  beforeEach(async function () {
    ...
    const fakeCLRegistry = await new MockCLRegistry__factory(fixture.admin1).deploy();

    await registerContract(fixture.registry, "CLRegistry", fakeCLRegistry.target);
  });
  ...
  describe("StakingTracker Initialize", function () {
    ...
    it("Check AddressBook setting", async function () {
      ...
      expect(allAddressInfo[0]).to.equalAddrList([
        await argsForContractA.nodeId.getAddress(),
        await argsForContractB.nodeId.getAddress(),
        await argsForContractC.nodeId.getAddress(),
        await argsForContractD.nodeId.getAddress(),
      ]);
      expect(allAddressInfo[1]).to.equalAddrList([
        await cnStakingV1A.getAddress(),
        await cnStakingV2B.getAddress(),
        await cnStakingV2C.getAddress(),
        await cnStakingV2D.getAddress(),
      ]);
      expect(allAddressInfo[2]).to.equalAddrList([
        await argsForContractA.rewardAddr.getAddress(),
        await argsForContractB.rewardAddr.getAddress(),
        await argsForContractC.rewardAddr.getAddress(),
        await argsForContractD.rewardAddr.getAddress(),
      ]);
    });
  ...
  describe("Test create/expire trackers", function () {
    ...
    it("Create and expire a single tracker", async function () {
      ...
      await expect(stakingTracker.refreshStake(cnStakingV2B.target)).to.emit(stakingTracker, "RetireTracker");
  ...
  describe("Test functions by calling directly", function () {
    describe("#refreshStake", function () {
      it("#refreshStake: When all trackers are live", async function () {
        ...
        // 4. Call refreshStake
        await expect(stakingTracker.refreshStake(cnStakingV2B.target)).to.emit(stakingTracker, "RefreshStake");
    ...
    describe("#refreshVoter", function () {
      it("#refreshVoter: Wrong staking contract", async function () {
        ...
        await expect(stakingTracker.refreshVoter(cnStakingV1A.target)).to.be.revertedWith("Invalid CnStaking contract");

        // 2. Not a staking contract
        await expect(stakingTracker.refreshVoter(stakingTracker.target)).to.be.revertedWith("Not a staking contract");
      });
      it("#refreshVoter: Emits RefreshVoter event", async function () {
        ...
        await expect(stakingTracker.refreshVoter(cnStakingV2B.target)).to.emit(stakingTracker, "RefreshVoter");
      });
    });
  });
  ...
  describe("4. Test functions by calling through CnStakingV2.sol", function () {
    ...
    describe("#refreshVoter", async function () {
      it("#updateVoterAddress", async function () {
        ...
        let cnStakingState = await stakingTracker.readCnStaking(cnStakingV2D.target);

        // We haven't set voter address yet
        expect(cnStakingState[4]).to.equal(ZeroAddress);
        ...
        cnStakingState = await stakingTracker.readCnStaking(cnStakingV2D.target);

        // Now voter address should be other1.address
        expect(cnStakingState[4]).to.equal(other1.address);

        await expect(stakingTracker.refreshVoter(cnStakingV2D.target)).to.emit(stakingTracker, "RefreshVoter");
