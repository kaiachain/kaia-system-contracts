...
describe("CLRegistry add/remove/update", function () {
  ...
  it("Add pair", async function () {
    ...
    await expect(clRegistry.addCLPair([{ nodeId, gcId, clPool }])).to.emit(clRegistry, "RegisterPair");

    // Invalid pair input
    await expect(clRegistry.addCLPair([{ nodeId, gcId: 0, clPool }])).to.be.revertedWith(
      "CLRegistry::addCLPair: Invalid pair input",
    );

    // Duplicated registration
    await expect(clRegistry.addCLPair([{ nodeId, gcId, clPool }])).to.be.revertedWith(
      "CLRegistry::addCLPair: GC ID does exist",
    );

    // ownership check
    const [, unknown] = await ethers.getSigners();
    await expect(clRegistry.connect(unknown).addCLPair([{ nodeId, gcId, clPool }])).to.be.revertedWithCustomError(
      clRegistry,
  ...
  it("Update pair", async function () {
    ...
    await expect(clRegistry.updateCLPair([{ nodeId, gcId, clPool: newCLPool }])).to.emit(clRegistry, "UpdatePair");

    expect(await clRegistry.getAllCLs()).to.deep.equal([[nodeId], [gcId], [newCLPool]]);

    // Invalid pair input
    await expect(clRegistry.updateCLPair([{ nodeId, gcId: 0, clPool }])).to.be.revertedWith(
      "CLRegistry::updateCLPair: Invalid pair input",
    );

    // Try to update not existing pair
    await expect(clRegistry.updateCLPair([{ nodeId, gcId: gcId + 1, clPool }])).to.be.revertedWith(
      "CLRegistry::updateCLPair: GC ID does not exist",
    );

    // ownership check
    const [, unknown] = await ethers.getSigners();
    await expect(clRegistry.connect(unknown).updateCLPair([{ nodeId, gcId, clPool }])).to.be.revertedWithCustomError(
      clRegistry,
      "OwnableUnauthorizedAccount",
    );
  });
