...
import "./interfaces/ITreasuryRebalance.sol";
...
contract TreasuryRebalance is Ownable, ITreasuryRebalance {
     ...
     */
    function registerRetired(address _retiredAddress) public onlyOwner onlyAtStatus(Status.Initialized) {
     ...
     * @param _retiredAddress is the address of the retired
     */
    function removeRetired(address _retiredAddress) public onlyOwner onlyAtStatus(Status.Initialized) {
     ...
     */
    function registerNewbie(address _newbieAddress, uint256 _amount) public onlyOwner onlyAtStatus(Status.Initialized) {
        require(!newbieExists(_newbieAddress), "Newbie address is already registered");
     ...
     * @param _newbieAddress is the address of the newbie
     */
    function removeNewbie(address _newbieAddress) public onlyOwner onlyAtStatus(Status.Initialized) {
    ...
    function approve(address _retiredAddress) public onlyAtStatus(Status.Registered) {
        require(retiredExists(_retiredAddress), "retired needs to be registered before approval");
     ...
     */
    function _getState(address _retiredAddress) private view returns (address[] memory adminList, uint256 req) {
     ...
     */
    function _updateApprover(address _retiredAddress, address _approver) private {
        ...
        emit Approved(_retiredAddress, _approver, retirees[index].approvers.length);
    }
     ...
     */
    function finalizeRegistration() public onlyOwner onlyAtStatus(Status.Initialized) {
     ...
     */
    function finalizeContract(string memory _memo) public onlyOwner onlyAtStatus(Status.Approved) {
        memo = _memo;
    ...
    function reset() public onlyOwner {
        ...
        require(
            ((status != Status.Finalized) && (block.number < rebalanceBlockNumber)),
     ...
     * @param _retiredAddress is the address of the retired
     */
    function getRetired(address _retiredAddress) public view returns (address, address[] memory) {
        uint256 index = getRetiredIndex(_retiredAddress);
        require(index != type(uint256).max, "Retired not registered");
     ...
     * @param _retiredAddress is the address of the retired
     */
    function getRetiredIndex(address _retiredAddress) public view returns (uint256) {
        for (uint256 i = 0; i < retirees.length; i++) {
            if (retirees[i].retired == _retiredAddress) {
                return i;
            }
     ...
     */
    function getNewbie(address _newbieAddress) public view returns (address, uint256) {
     ...
     * @param _newbieAddress is the address of the newbie
     */
    function getNewbieIndex(address _newbieAddress) public view returns (uint256) {
        for (uint256 i = 0; i < newbies.length; i++) {
            if (newbies[i].newbie == _newbieAddress) {
                return i;
            }
