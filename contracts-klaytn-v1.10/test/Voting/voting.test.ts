...
import { CnStakingV2 } from "../../typechain-types";
...
describe("Voting.sol", function () {
  ...
  describe("Voting Initialize", function () {
    it("Check constructor", async function () {
      ...
      expect(stakingTrackerAddr).to.equal(stakingTracker.target);
    ...
    describe("Tests which need to be done after proposal", function () {
      ...
      describe("#castVote", function () {
        ...
        it("#castVote: Need to have votes", async function () {
          ...
          await submitAndExecuteRequest(
            cnStakingV2A as CnStakingV2,
            [admin1],
            1,
            FuncID.WithdrawLockupStaking,
            admin1,
            [0, FuncID.WithdrawLockupStaking, other1.address, BigInt(toPeb(2_000_000n)), 0],
          );
      ...
      describe("#execute", function () {
        ...
        it("#execute: Successfully execute proposal", async function () {
          ...
          expect(other1After - other1Before).to.equal(toPeb(300n));
        });
      });
    });
    describe("Tests for updating contract parameters", function () {
      async function goToQueueState(voter: Signer[], choice: number) {
        const { voting, secretary } = fixture;
  ...
  describe("Test votes count and power", function () {
    ...
    it("Withdraw stakes in pending state", async function () {
      ...
      // Now A has no votes
      await submitAndExecuteRequest(cnStakingV2A as CnStakingV2, [admin1], 1, FuncID.WithdrawLockupStaking, admin1, [
        0,
        FuncID.WithdrawLockupStaking,
        other1.address,
