...
import { nowBlock, jumpBlock } from "../../helpers/utils";
...
describe("GovParam", function () {
  ...
  const defaultParam = [BigInt("0"), false, emptyVal];
  ...
  describe("checkpoints", function () {
    it("checkpoints success", async function () {
      ...
      await expect(gp.setParam(name, true, val1, activation))
        .to.emit(gp, "SetParam")
        .withArgs(name, true, val1, activation);
      expect(await gp.checkpoints(name)).to.deep.equal([defaultParam, [BigInt(activation), true, val1]]);
    });
  });
    ...
    it("getAllCheckpoints success", async function () {
      ...
      for (const [i, name] of names.entries()) {
        ...
        for (let j = 0; j < 3; j++) {
          ...
          await jumpBlock(20);
          p.push([BigInt(activation), true, val]);
        }
        expected.push(p);
      }
