...
import { CnStakingV2, Voting__factory } from "../../../typechain-types";
...
export async function votingTestFixture() {
  ...
  const voting = await new Voting__factory(contractValidator).deploy(ZeroAddress, secretary.address);
  // Give voting contract KLAY for testing
  await setBalance(await voting.getAddress(), "0x52B7D2DCC80CD2E4000000");
  ...
  const stakingTrackerByteCode = await ethers.provider.getCode(stakingTracker.target);
  ...
  const cnStakingV2A = (await deployAndInitStakingContract(2, argsForContractA)) as unknown as CnStakingV2;
  const cnStakingV2B = (await deployAndInitStakingContract(2, argsForContractB)) as unknown as CnStakingV2;
  const cnStakingV2C = (await deployAndInitStakingContract(2, argsForContractC)) as unknown as CnStakingV2;
  const cnStakingV2D = (await deployAndInitStakingContract(2, argsForContractD)) as unknown as CnStakingV2;
  ...
  await addressBook.mockRegisterCnStakingContracts(
    ...
    [cnStakingV2A.target, cnStakingV2B.target, cnStakingV2C.target, cnStakingV2D.target],
  ...
  const iface = new ethers.Interface(ABIForVotingUpdate);
  ...
  const txUpdateStakingTracker: tx = {
    target: votingAddr,
    value: "0",
    calldata: iface.encodeFunctionData("updateStakingTracker", [stakingTracker2.target]),
  };
