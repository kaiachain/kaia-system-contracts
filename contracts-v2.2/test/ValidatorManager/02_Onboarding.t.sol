...
import {
    BaseTest,
    PublicDelegationFactoryV2,
    ICnStakingV3MultiSig,
    IValidatorManager,
    IKIP113
} from "./base/ValidatorManager.t.sol";
...
contract OnboardingTest is BaseTest {
    ...
    function test_requestOnboarding_revert_invalidConsensusNodeId() public {
        bytes memory requestData = _prepareOnboardingRequest(onboardingNode);
        vm.expectRevert(IValidatorManager.UnmatchedConsensusNodeId.selector);
        vm.prank(manager);
        validatorManager.request(vm.randomAddress(), IValidatorManager.RequestType.Onboarding, requestData);
    }
