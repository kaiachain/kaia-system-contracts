...
contract CnStakingContract {
    ...
    constructor(
        ...
        address[] _cnAdminlist,
        uint256 _requirement,
        uint256[] _unlockTime,
        uint256[] _unlockAmount
    ) public validRequirement(_cnAdminlist.length, _requirement) notNull(_nodeId) notNull(_rewardAddress) {
    ...
    function reviewInitialConditions() external onlyAdmin(msg.sender) beforeInit {
    ...
    function submitUpdateRequirement(
        uint256 _requirement
    ) external afterInit onlyAdmin(msg.sender) validRequirement(adminList.length, _requirement) {
    ...
    function withdrawLockupStaking(address _to, uint256 _value) external onlyMultisigTx {
        uint256 withdrawableStakingAmount;
        (, , , , withdrawableStakingAmount) = getLockupStakingInfo();
    ...
    function approveStakingWithdrawal(address _to, uint256 _value) external onlyMultisigTx {
    ...
    function cancelApprovedStakingWithdrawal(uint256 _approvedWithdrawalId) external onlyMultisigTx {
        require(withdrawalRequestMap[_approvedWithdrawalId].to != 0, "Withdrawal request does not exist.");
        require(withdrawalRequestMap[_approvedWithdrawalId].state == WithdrawalStakingState.Unknown, "Invalid state.");
    ...
    function updateRewardAddress(address _rewardAddress) external onlyMultisigTx {
        rewardAddress = _rewardAddress;
    ...
    function() external payable afterInit {
        require(msg.value > 0, "Invalid amount.");
        staking = staking.add(msg.value);
        emit StakeKlay(msg.sender, msg.value);
    }
    ...
    function withdrawApprovedStaking(uint256 _approvedWithdrawalId) external onlyAdmin(msg.sender) {
        require(withdrawalRequestMap[_approvedWithdrawalId].to != 0, "Withdrawal request does not exist.");
        require(withdrawalRequestMap[_approvedWithdrawalId].state == WithdrawalStakingState.Unknown, "Invalid state.");
    ...
    function executeRequest(uint256 _id) private {
        ...
        if (requestMap[_id].functionId == Functions.WithdrawLockupStaking) {
            ...
            executed = address(this).call(
                0x505ebed4,
                address(requestMap[_id].firstArg),
     ...
     * Getter functions
     */
    function getReviewers() external view beforeInit returns (address[]) {
    ...
    function getRequestInfo(
        uint256 _id
    ) external view returns (Functions, bytes32, bytes32, bytes32, address, address[], RequestState) {
        return (
            requestMap[_id].functionId,
            requestMap[_id].firstArg,
            requestMap[_id].secondArg,
    ...
    function getLockupStakingInfo() public view afterInit returns (uint256[], uint256[], uint256, uint256, uint256) {
    ...
    function getApprovedStakingWithdrawalInfo(
        uint256 _index
    ) external view returns (address, uint256, uint256, WithdrawalStakingState) {
        return (
    ...
    function getState()
        external
        view
        returns (address, address, address, address[], uint256, uint256[], uint256[], bool, bool)
    {
        return (
            contractValidator,
            nodeId,
