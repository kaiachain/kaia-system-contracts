...
contract AddressBook {
    ...
    function constructContract(
        ...
        // Mainnet address 0x854CA8508C8BE2bb1f3C244045786410Cb7D5D0a
        // Kairos address 0xfcd0ea52081a6d0c75ffddb035a91a782f4af027
        require(msg.sender == 0x1234567890123456789012345678901234567890, "Invalid sender."); // Only hardcoded specific address can initialize
    ...
    function executeRequest(bytes32 _id) private {
        ...
        if (_executeRequest.functionId == Functions.AddAdmin) {
            ...
            executed = address(this).call(
                0xc7e9de75,
                address(_executeRequest.firstArg),
                uint256(_executeRequest.secondArg)
            );
            ...
            executed = address(this).call(
                0x298b3c61,
                address(_executeRequest.firstArg),
                address(_executeRequest.secondArg),
                address(_executeRequest.thirdArg)
            );
    ...
    function submitRegisterCnStakingContract(
        ...
        bytes32 id = getId(
            ...
            bytes32(_cnNodeId),
            bytes32(_cnStakingContractAddress),
            bytes32(_cnRewardAddress)
        );
    ...
    function getRequestInfo(
        bytes32 _id
    ) external view returns (Functions, bytes32, bytes32, bytes32, address[], uint256, RequestState) {
        return (
    ...
    function getRequestInfoByArgs(
        ...
        bytes32 _secondArg,
        bytes32 _thirdArg
    ) external view returns (bytes32, address[], uint256, RequestState) {
