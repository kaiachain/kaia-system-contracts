...
contract Operator is Initializable, ReentrancyGuardUpgradeable, UUPSUpgradeable, IERC165, IOperator {
    ...
    function initialize(
        ...
        require(
            IERC165(initGuardian).supportsInterface(type(IGuardian).interfaceId),
    ...
    function addOperator(
        address operator
    ) public override onlyGuardian operatorDoesNotExist(operator) notNull(operator) {
    ...
    function removeOperator(address operator) public override onlyGuardian operatorExists(operator) {
    ...
    function replaceOperator(
        address operator,
        address newOperator
    ) public override onlyGuardian operatorExists(operator) operatorDoesNotExist(newOperator) notNull(newOperator) {
    ...
    function changeBridge(address newBridge) public override onlyGuardian notNull(newBridge) {
    ...
    ) public override onlyGuardian validRequirement(operators.length, _minOperatorRequiredConfirm) {
    ...
    function submitTransaction(
        ...
        try IBridge(bridge).bytes2Provision(data) returns (IBridge.ProvisionData memory provision) {
            require(
                bridge == to,
                "KAIA::Operator: Provision transaction must be targeted to known bridge contract address"
            );
    ...
    function confirmTransaction(uint64 txID) public override txExists(txID) notConfirmed(txID, msg.sender) {
    ...
    function revokeConfirmation(
        uint64 txID
    ) public override operatorExists(msg.sender) confirmed(txID, msg.sender) notExecuted(txID) {
        confirmations[txID][msg.sender] = false;
        emit Revocation(msg.sender, txID);
    }

    /// @dev See {IOperator-executeTransaction}
    function executeTransaction(uint64 txID) public override operatorExists(msg.sender) confirmed(txID, msg.sender) {
    ...
    /// @param data Calldata
    function predefinedExecute(address to, bytes memory data) private nonReentrant returns (bool) {
    ...
    function addTransaction(address to, bytes memory data, uint256 uniqUserTxIndex) internal returns (uint64) {
    ...
    function getConfirmationCount(uint64 txID) public view override returns (uint64) {
        uint64 count = 0;
        for (uint64 i = 0; i < operators.length; i++) {
            if (confirmations[txID][operators[i]]) {
                count += 1;
    ...
    function getTransactionCount(bool pending, bool executed) public view override returns (uint64, uint64) {
        uint64 pendingCnt = 0;
    ...
    function getConfirmations(uint64 txID) public view override returns (address[] memory) {
    ...
    function getUnconfirmedProvisionSeqs(
        ...
        uint64 range
    ) public view override operatorExists(targetOperator) returns (uint64[] memory) {
    ...
    function updateNextUnsubmittedSeq(uint64 nextUnprovisionedSeq) public override operatorExists(msg.sender) {
    ...
    function getSeq2TxIDs(uint64 seq) public view override returns (uint64[] memory) {
        return EnumerableSetUint64.getAll(seq2TxID[seq]);
    }

    /// @dev See {IOperator-checkProvisionShouldSubmit}
    function checkProvisionShouldSubmit(bytes32 hashedData, address operator) public view override returns (bool) {
