...
import "../libraries/EnumerableSetUint64.sol";
...
abstract contract IBridge {
    ...
    function isProvisioned(uint64 seq) external view virtual returns (bool);

    /// @dev A list version of the `isProvisioned`
    /// @param from start seqeunce number
    /// @param to end seqeunce number
    function isProvisionedRange(uint64 from, uint64 to) external view virtual returns (uint64[] memory);
    ...
    function bytes2Provision(bytes calldata data) external pure virtual returns (ProvisionData memory);
    ...
    /// @dev Get all locked
    function getAllSwapRequests() external view virtual returns (SwapRequest[] memory);

    /// @dev Get claim ranges
    /// @param from Range start number
    /// @param to Range end number
    function getSwapRequests(uint256 from, uint256 to) external view virtual returns (SwapRequest[] memory);

    /// @dev Return `claimCandidates` set
    function getClaimCandidates() external view virtual returns (uint64[] memory);

    /// @dev Return `claimCandidates` set size
    function getClaimCandidatesSize() external view virtual returns (uint256);

    /// @dev Return `claimCandidates` set with the given range
    function getClaimCandidatesRangePure(uint64 range) external view virtual returns (uint64[] memory);

    /// @dev Return `claimCandidates` set with the given range
    /// @param range Range of set
    function getClaimCandidatesRange(uint64 range) external view virtual returns (uint64[] memory);

    /// @dev Return `claimFailures` set
    function getClaimFailures() external view virtual returns (uint64[] memory);

    /// @dev Return `claimFailures` set
    /// @param range Range of set
    function getClaimFailuresRange(uint64 range) external view virtual returns (uint64[] memory);

    //////////////////// Constant ////////////////////
    uint256 public constant KAIA_UNIT = 10e18;
