...
abstract contract IOperator {
    ...
    modifier validRequirement(uint256 operatorCount, uint64 _minOperatorRequiredConfirm) {
        require(_minOperatorRequiredConfirm <= operatorCount && _minOperatorRequiredConfirm != 0 && operatorCount != 0);
        _;
    }
    ...
    function isConfirmed(uint64 txID) external view virtual returns (bool);

    /// @dev Returns number of confirmations of a transaction.
    /// @param txID Transaction ID.
    /// @return Number of confirmations.
    function getConfirmationCount(uint64 txID) external view virtual returns (uint64);

    /// @dev Returns total number of transactions after filers are applied.
    /// @param pending Include pending transactions.
    /// @param executed Include executed transactions.
    /// @return Total number of transactions after filters are applied.
    function getTransactionCount(bool pending, bool executed) external view virtual returns (uint64, uint64);

    /// @dev Returns list of operators.
    /// @return List of operator addresses.
    function getOperators() external view virtual returns (address[] memory);

    /// @dev Returns array with operator addresses, which confirmed transaction.
    /// @param txID Transaction ID.
    /// @return Returns array of operator addresses.
    function getConfirmations(uint64 txID) external view virtual returns (address[] memory);
    ...
    function getTransactionIds(
        ...
        bool pending,
        bool executed
    ) external view virtual returns (uint64[] memory, uint64[] memory);
    ...
    /// @param seq Sequence number
    function getSeq2TxIDs(uint64 seq) external view virtual returns (uint64[] memory);
    ...
    function checkProvisionShouldSubmit(bytes32 hashedData, address operator) external view virtual returns (bool);
