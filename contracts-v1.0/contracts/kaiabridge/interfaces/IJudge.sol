...
abstract contract IJudge {
    ...
    modifier validRequirement(uint256 judgeCount, uint256 _minJudgeRequiredConfirm) {
        require(_minJudgeRequiredConfirm <= judgeCount && _minJudgeRequiredConfirm != 0 && judgeCount != 0);
        _;
    }
    ...
    function isConfirmed(uint256 txID) external view virtual returns (bool);

    /// @dev Returns number of confirmations of a transaction.
    /// @param txID Transaction ID.
    /// @return Number of confirmations.
    function getConfirmationCount(uint256 txID) external view virtual returns (uint256);

    /// @dev Returns total number of transactions after filers are applied.
    /// @param pending Include pending transactions.
    /// @param executed Include executed transactions.
    /// @return Total number of transactions after filters are applied.
    function getTransactionCount(bool pending, bool executed) external view virtual returns (uint256, uint256);

    /// @dev Returns list of judges.
    /// @return List of judge addresses.
    function getJudges() external view virtual returns (address[] memory);

    /// @dev Returns array with judge addresses, which confirmed transaction.
    /// @param txID Transaction ID.
    /// @return Returns array of judge addresses.
    function getConfirmations(uint256 txID) external view virtual returns (address[] memory);
