...
contract Guardian is Initializable, ReentrancyGuardUpgradeable, UUPSUpgradeable, IERC165, IGuardian {
    ...
    function initialize(address[] calldata initGuardians, uint8 _minGaurdianRequiredConfirm) public initializer {
    ...
    function addGuardian(
        address guardian
    ) public override onlyGuardian guardianDoesNotExist(guardian) notNull(guardian) {
    ...
    function removeGuardian(address guardian) public override onlyGuardian guardianExists(guardian) {
    ...
    function replaceGuardian(
        address guardian,
        address newGuardian
    ) public override onlyGuardian guardianExists(guardian) guardianDoesNotExist(newGuardian) notNull(newGuardian) {
    ...
    ) public override onlyGuardian validRequirement(guardians.length, _minGuardianRequiredConfirm) {
    ...
    function confirmTransaction(uint256 txID) public override txExists(txID) notConfirmed(txID, msg.sender) {
    ...
    function revokeConfirmation(
        uint256 txID
    ) public override guardianExists(msg.sender) confirmed(txID, msg.sender) notExecuted(txID) {
        confirmations[txID][msg.sender] = false;
        emit Revocation(msg.sender, txID);
    }

    /// @dev See {IGuardian-executeTransaction}
    function executeTransaction(uint256 txID) public override guardianExists(msg.sender) confirmed(txID, msg.sender) {
    ...
    function addTransaction(address to, bytes memory data, uint256 uniqUserTxIndex) internal returns (uint256) {
    ...
    function getConfirmationCount(uint256 txID) public view override returns (uint256) {
        uint256 count = 0;
        for (uint256 i = 0; i < guardians.length; i++) {
            if (confirmations[txID][guardians[i]]) {
                count += 1;
    ...
    function getTransactionCount(bool pending, bool executed) public view override returns (uint256, uint256) {
    ...
    function getConfirmations(uint256 txID) public view override returns (address[] memory) {
