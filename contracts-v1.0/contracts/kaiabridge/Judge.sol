...
contract Judge is Initializable, ReentrancyGuardUpgradeable, UUPSUpgradeable, IERC165, IJudge {
    ...
    function addJudge(address judge) public override onlyGuardian judgeDoesNotExist(judge) notNull(judge) {
        isJudge[judge] = true;
        judges.push(judge);
    ...
    function removeJudge(address judge) public override onlyGuardian judgeExists(judge) {
        isJudge[judge] = false;
    ...
    function replaceJudge(
        address judge,
        address newJudge
    ) public override onlyGuardian judgeExists(judge) judgeDoesNotExist(newJudge) notNull(newJudge) {
    ...
    function changeRequirement(
        uint8 _minJudgeRequiredConfirm
    ) public override onlyGuardian validRequirement(judges.length, _minJudgeRequiredConfirm) {
        minJudgeRequiredConfirm = _minJudgeRequiredConfirm;
    ...
    function revokeConfirmation(
        uint256 txID
    ) public override judgeExists(msg.sender) confirmed(txID, msg.sender) notExecuted(txID) {
        confirmations[txID][msg.sender] = false;
        emit Revocation(msg.sender, txID);
    }

    /// @dev See {IJudge-executeTransaction}
    function executeTransaction(uint256 txID) public override judgeExists(msg.sender) confirmed(txID, msg.sender) {
