...
contract KAIABridge is Initializable, ReentrancyGuardUpgradeable, UUPSUpgradeable, IERC165, IBridge {
    ...
    ) public initializer notNull(initOperator) notNull(initGuardian) notNull(initJudge) {
        require(
            IERC165(initOperator).supportsInterface(type(IOperator).interfaceId),
    ...
    function provision(ProvisionData calldata prov) public override onlyOperator notPause {
        uint64 seq = prov.seq;
        ...
        emit ProvisionConfirm(
            ProvisionConfirmedEvent({seq: seq, sender: prov.sender, receiver: prov.receiver, amount: prov.amount})
        );
    ...
    function transfer(string calldata receiver) public payable override nonReentrant notPause transferEnable {
        if (addrValidationOn) {
    ...
    /// @param revertOnFail Make reverts if operations fails and the value is true, otherwise no make revert, but record its failure
    function claim(ProvisionData memory prov, bool revertOnFail) internal nonReentrant returns (bool) {
        ...
        (bool sent, ) = prov.receiver.call{value: prov.amount, gas: gasleft() / 2}("");
        if (!sent) {
            if (revertOnFail) {
    ...
    function isPassedTimeLockDuration(uint256 seq) public view override returns (bool) {
    ...
    function isProvisionedRange(uint64 from, uint64 to) public view override returns (uint64[] memory) {
    ...
    function getAllSwapRequests() public view override returns (SwapRequest[] memory) {
        return locked;
    }

    /// @dev See {IBridge-getSwapRequests}
    function getSwapRequests(uint256 from, uint256 to) public view override returns (SwapRequest[] memory) {
    ...
    function bytes2Provision(bytes calldata data) external pure override returns (ProvisionData memory) {
        return abi.decode(data[4:], (ProvisionData));
    }

    // @dev See {IBridge-getClaimCandidates}
    function getClaimCandidates() public view override returns (uint64[] memory) {
    ...
    function getClaimCandidatesRangePure(uint64 range) public view override returns (uint64[] memory) {
        return EnumerableSetUint64.getRange(claimCandidates, range);
    }

    // @dev See {IBridge-getClaimCandidatesRange}
    function getClaimCandidatesRange(uint64 range) public view override returns (uint64[] memory) {
    ...
    function getClaimFailuresRange(uint64 range) public view override returns (uint64[] memory) {
