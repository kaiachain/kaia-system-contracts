...
import { MockPublicDelegation, MockPublicDelegation__factory } from "../../typechain-types";
import { keccak256, parseEther, toUtf8Bytes } from "ethers";
import hre from "hardhat";

const ADMIN_ROLE = keccak256(toUtf8Bytes("ADMIN_ROLE"));
const SECRETARY_ROLE = keccak256(toUtf8Bytes("SECRETARY_ROLE"));
...
describe("Airdrop", function () {
  ...
  beforeEach(async function () {
    ...
    await setBalance(await lockup.getAddress(), totalDelegatedAmount);
  });
  ...
  describe("Acquisition", function () {
    ...
    describe("#withdrawAcquisition", function () {
      const requestAmount = parseEther("1000");
      this.beforeEach(async function () {
        const { lockup, admin } = fixture;
      ...
      it("successfully withdraw acquisition request", async function () {
        ...
        expect(afterBalance).to.be.closeTo(beforeBalance + requestAmount, parseEther("0.0001"));
        const acquisition = await lockup.getAcquisition(0);
  ...
  describe("Delegated Transfer", function () {
    ...
    describe("#withdrawDelegatedTransfer", function () {
      ...
      it("successfully withdraw delegated transfer", async function () {
        ...
        expect(afterBalance).to.be.equal(beforeBalance + requestAmount);
  ...
  describe("Interact with public delegation", function () {
    let mockPD: MockPublicDelegation;
    this.beforeEach(async function () {
      const { lockup, admin } = fixture;

      await lockup.connect(admin).refreshDelegated();

      mockPD = await new MockPublicDelegation__factory(admin).deploy();
      await mockPD.mockSetMaxRedeem(await lockup.getAddress(), 1000);

      await mockPD.mockSetRequestIdToOwner(0, await lockup.getAddress());
    });
    it("#withdrawStakingAmounts: successfully withdraw staking amounts", async function () {
      const { lockup, admin } = fixture;

      await expect(lockup.connect(admin).withdrawStakingAmounts(await mockPD.getAddress(), 1000))
        .to.emit(lockup, "WithdrawStakingAmounts")
        .withArgs(await mockPD.getAddress(), 1000);
    });
    it("#withdrawStakingAmounts: failed to withdraw more than max redeemable shares", async function () {
      const { lockup, admin } = fixture;

      await expect(lockup.connect(admin).withdrawStakingAmounts(await mockPD.getAddress(), 10_000)).to.be.revertedWith(
    ...
    it("#claimStakingAmounts: successfully claim staking amounts", async function () {
      ...
      await expect(lockup.connect(admin).claimStakingAmounts(await mockPD.getAddress(), 0))
        .to.emit(lockup, "ClaimStakingAmounts")
        .withArgs(await mockPD.getAddress(), 0);
    });
    it("#claimStakingAmounts: failed to claim staking of not owned public delegation", async function () {
      const { lockup, admin, user } = fixture;

      await mockPD.mockSetRequestIdToOwner(0, user.address); // Not owned

      await expect(lockup.connect(admin).claimStakingAmounts(await mockPD.getAddress(), 0)).to.be.revertedWith(
