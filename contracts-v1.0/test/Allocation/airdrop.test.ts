...
describe("Airdrop", function () {
  ...
  beforeEach(async function () {
    ...
    await setBalance(await airdrop.getAddress(), totalAirdropAmount);
  });
  ...
  describe("Claim airdrop", function () {
    ...
    it("#claim: successfully get airdrop", async function () {
      ...
      expect(afterBalance - beforeBalance).to.be.closeTo(claimInfo[0].amount, parseEther("0.0001"));
    ...
    it("#claimFor: successfully get airdrop", async function () {
      ...
      const afterBalance = await hre.ethers.provider.getBalance(claimers[0].address);
      expect(afterBalance - beforeBalance).to.equal(claimInfo[0].amount);
      expect(await airdrop.claimed(claimers[0].address)).to.be.true;
    });
    ...
    it("#claimBatch: successfully get airdrop", async function () {
      ...
      for (let i = 0; i < claimers.length; i++) {
        expect(afterBalances[i] - beforeBalances[i]).to.equal(claimInfo[i].amount);
    ...
    it("#claimBatch: failed claim", async function () {
      ...
      await airdrop.addClaim(await noReceiverContract.getAddress(), claimInfo[0].amount);

      const beforeBalance = await hre.ethers.provider.getBalance(await airdrop.getAddress());

      await expect(airdrop.connect(notClaimer).claimFor(await noReceiverContract.getAddress())).to.be.revertedWith(
        "Airdrop: claim failed",
      );
      expect(await airdrop.claimed(await noReceiverContract.getAddress())).to.be.false;
      expect(await hre.ethers.provider.getBalance(await airdrop.getAddress())).to.equal(beforeBalance);
    });
  });
  describe("Divide airdrop contract", function () {
    it("#register airdrop contract as claimer", async function () {
      ...
      await airdrop.addClaim(await newAirdrop.getAddress(), parseEther("100"));

      await airdrop.toggleClaimAllowed();

      await airdrop.claimFor(await newAirdrop.getAddress());

      expect(await airdrop.claimed(await newAirdrop.getAddress())).to.be.true;
      expect(await hre.ethers.provider.getBalance(await newAirdrop.getAddress())).to.equal(parseEther("100"));
    });
  });
