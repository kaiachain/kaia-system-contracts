...
import { nowTime, toPeb, addressBookFixture } from "../../../helpers/utils";
...
export const ROLES = {
  ...
  STAKER_ROLE: keccak256(toUtf8Bytes("STAKER_ROLE")),
  UNSTAKING_APPROVER_ROLE: keccak256(toUtf8Bytes("UNSTAKING_APPROVER_ROLE")),
  UNSTAKING_CLAIMER_ROLE: keccak256(toUtf8Bytes("UNSTAKING_CLAIMER_ROLE")),
};
...
export async function cnV3MultiSigUnitTestFixture() {
  ...
  // Deploy contracts
  const cnStakingV3 = await new CnStakingV3MultiSig__factory(contractValidator).deploy(
  ...
  await addressBook.registerCnStakingContract(nodeId.address, cnStakingV3.target, rewardAddr.address);
...
export async function cnV3MultiSigPublicDelegationTestFixture() {
  ...
  await cnStakingV3.setStakingTracker(stakingTracker.target);
  await cnStakingV3.setGCId(gcId);

  await cnStakingV3.setPublicDelegation(pdFactory.target, pdParam);
...
export async function cnV3InitialLockupNotDepositedFixture() {
  ...
  await cnV3.setStakingTracker(stakingTracker.target);
  await cnV3.setGCId(1);
  await cnV3.reviewInitialConditions();

  await addressBook.constructContract([deployer.address], 1);
  await addressBook.registerCnStakingContract(nodeId.address, cnV3.target, rewardAddress.address);
...
export async function cnV3PublicDelegationNotRegisteredFixture() {
  ...
  for (let i = 0; i < gcId.length; i++) {
    cnV3s.push(await new CnStakingV3__factory(deployer).deploy(deployer.address, node[i], ZeroAddress, [], []));
    await cnV3s[i].setStakingTracker(stakingTracker.target);
    await cnV3s[i].setGCId(gcId[i]);
  }

  const testingPsParam = new ethers.AbiCoder().encode(
    ["tuple(address, address,  uint256, string)"],
