...
import { CnStakingV2__factory } from "@kaiachain/system-contracts-klaytn-v1.10/typechain-types";
import { CnStakingV3__factory, PublicDelegation__factory } from "../../../typechain-types";
import { loadFixture } from "@nomicfoundation/hardhat-network-helpers";
import { gcId, unlockTime, unlockAmount } from "../../../helpers/utils";
...
export async function cnV3PublicDelegationFixture() {
  ...
  for (let i = 0; i < gcId.length; i++) {
    ...
    await cnV3s[i].setPublicDelegation(pdFactory.target, pdParam);
  ...
  await addressBook.mockRegisterCnStakingContracts(
    ...
    [...cnV3s.map((c) => c.target), cnV3WithInitialLockup.target, cnV2.target],
