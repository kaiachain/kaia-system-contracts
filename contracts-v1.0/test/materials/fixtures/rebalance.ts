...
import { setCodeAt } from "../../../helpers/utils";
import { parseEther } from "ethers";
import { TreasuryRebalanceV2__factory } from "../../../typechain-types";

export const executionBlock = 200;
export const value = parseEther("20");
export const memo =
...
export async function rebalanceTestFixture() {
  ...
  await zeroed1.addAdmin(zeroedAdmins[0].address);
  await zeroed1.addAdmin(zeroedAdmins[1].address);
  await zeroed1.changeMinReq(2);
  ...
  await rebalance_manager.sendTransaction({ to: await zeroed1.getAddress(), value: value });

  const KEF = await ethers.getContractFactory("SenderTest1");
  const zeroed2 = await KEF.deploy();
  await zeroed2.addAdmin(zeroedAdmins[2].address);
  await zeroed2.addAdmin(zeroedAdmins[3].address);
  await zeroed2.changeMinReq(2);
  ...
  await rebalance_manager.sendTransaction({ to: await zeroed2.getAddress(), value: value });

  await setCodeAt("MockPoC", "0x38138d89c321b3b5f421e9452b69cf29e4380bae");
  const mockZeroed3 = await ethers.getContractAt("MockPoC", "0x38138d89c321b3b5f421e9452b69cf29e4380bae");

  const trV2 = await new TreasuryRebalanceV2__factory(rebalance_manager).deploy(executionBlock);
