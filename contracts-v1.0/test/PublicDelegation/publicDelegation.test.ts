...
import { cnV3PublicDelegationFixture } from "../materials";
import { addTime, augmentChai, DIRTY, toPeb, WEEK } from "../../helpers/utils";
import { MockCnStakingOverV2__factory, PublicDelegation } from "../../typechain-types";
...
describe("PublicDelegation tests", function () {
  ...
  describe("Operations", function () {
    it("#updateCommissionTo: only admin can update commissionTo", async function () {
      ...
      await expect(pd1.connect(user1).updateCommissionTo(user1.address)).to.be.revertedWithCustomError(
        pd1,
    ...
    it("#updateCommissionTo: clean up reward to previous commissionTo address", async function () {
      const { pd1, commissionTo, user1 } = await loadFixture(cnV3PublicDelegationFixture);
      const before = await ethers.provider.getBalance(commissionTo[0]);

      await pd1.updateCommissionRate(1000); // 10%

      // Get reward: 1000 KAIA.
      await setBalance(await pd1.getAddress(), parseEther("1000"));
      ...
      expect(await ethers.provider.getBalance(commissionTo[0])).to.be.eq(before + BigInt(toPeb(100n)));
    });
    it("#updateCommissionRate: only admin can update commissionRate", async function () {
      const { pd1, user1 } = await loadFixture(cnV3PublicDelegationFixture);
      await expect(pd1.connect(user1).updateCommissionRate(1)).to.be.revertedWithCustomError(
        pd1,
        "OwnableUnauthorizedAccount",
      );

      await expect(pd1.updateCommissionRate(1)).to.emit(pd1, "UpdateCommissionRate").withArgs(0, 1);
    ...
    it("#updateCommissionRate: updated commissionRate isn't applied to existing rewards", async function () {
      ...
      await expect(pd1.updateCommissionRate(1000)).to.emit(pd1, "UpdateCommissionRate").withArgs(0, 1000);
    ...
    it("#updateCommissionRate: can't set commissionRate to more than MAX_COMMISSION_RATE", async function () {
      ...
      await expect(pd1.updateCommissionRate(10001)).to.be.revertedWith("Commission rate is too high.");
    });
  });
  describe("Simple ERC20 test", function () {
    it("#transfer: not allowed", async function () {
      const { pd1, user1, user2 } = await loadFixture(cnV3PublicDelegationFixture);
      ...
      await expect(pd1.connect(user1).transfer(user2.address, toPeb(500n))).to.be.revertedWith("Transfer not allowed.");
    });
    it("#transferFrom: not allowed", async function () {
      const { pd1, user1, user2, user3 } = await loadFixture(cnV3PublicDelegationFixture);
      ...
      await expect(pd1.connect(user3).transferFrom(user1.address, user2.address, toPeb(500n))).to.be.revertedWith(
  ...
  describe("Simple delegation tests", function () {
    it("#stake: user can stake", async function () {
      const { cnV3s, pd1, user1, user2, user3 } = await loadFixture(cnV3PublicDelegationFixture);

      // User1 stakes 1000 KAIA.
      // Shares of user1 = 1000 * 10^18.
      await expect(pd1.connect(user1).stake({ value: toPeb(1000n) })).to.emit(pd1, "Staked");
      expect(await ethers.provider.getBalance(cnV3s[0].target)).to.be.eq(toPeb(1000n));
      ...
      await expect(pd1.connect(user2).stake({ value: toPeb(2000n) })).to.emit(pd1, "Staked");
      expect(await ethers.provider.getBalance(cnV3s[0].target)).to.be.eq(toPeb(3000n));
      ...
      await expect(pd1.connect(user3).stake({ value: toPeb(3000n) })).to.emit(pd1, "Staked");
      expect(await ethers.provider.getBalance(cnV3s[0].target)).to.be.eq(toPeb(6000n));
    ...
    it("#stake: user can't stake too small amounts (shares < 1)", async function () {
      const { cnV3s, pd1, user1, user2 } = await loadFixture(cnV3PublicDelegationFixture);
      ...
      await expect(pd1.connect(user2).stake({ value: 1 })).to.be.revertedWith("Stake amount is too low.");
    });
    it("#stakeFor: user can stake on behalf of other users", async function () {
      ...
      await expect(pd1.connect(user1).stakeFor(user2.address, { value: toPeb(1000n) }))
        .to.emit(pd1, "Staked")
      ...
      await expect(pd1.connect(user2).stakeFor(user3.address, { value: toPeb(2000n) }))
        .to.emit(pd1, "Staked")
    ...
    it("#stakeFor: user can't stake for zeor address", async function () {
      ...
      // User1 stakes 1000 KAIA on behalf of user2.
      // Shares of user2 = 1000 * 10^18.
      await expect(pd1.connect(user1).stakeFor(ZeroAddress, { value: toPeb(1000n) })).to.be.revertedWith(
        "Address is null.",
      );
    });
    it("#fallback: fallback leads to stakes", async function () {
      ...
      // User1 stakes 1000 KAIA.
      // Shares of user1 = 1000 * 10^18.
      await expect(user1.sendTransaction({ to: pd1.target, value: toPeb(1000n) }))
        .to.emit(pd1, "Staked")
        .withArgs(user1.address, toPeb(1000n), toPeb(1000n));
    ...
    it("#redeem: user can withdraw staked amount", async function () {
      const { cnV3s, pd1, user1 } = await loadFixture(cnV3PublicDelegationFixture);
    ...
    it("#redeem: user can't withdraw more than staked amount", async function () {
      ...
      await expect(pd1.connect(user1).redeem(user1.address, toPeb(1001n))).to.be.revertedWithCustomError(
        pd1,
    ...
    it("#redeem: user can't withdraw to zero address", async function () {
      ...
      await expect(pd1.connect(user1).redeem(ZeroAddress, toPeb(1000n))).to.be.revertedWith("Address is null.");
    });
    ...
    it("#withdraw: user can't withdraw more than staked amount", async function () {
      ...
      await expect(pd1.connect(user1).withdraw(user1.address, toPeb(1001n))).to.be.revertedWithCustomError(
        pd1,
        "ERC20InsufficientBalance",
      );
    });
    it("#withdraw: user can't withdraw to zero address", async function () {
      ...
      await expect(pd1.connect(user1).withdraw(ZeroAddress, toPeb(1001n))).to.be.revertedWith("Address is null.");
    });
    it("#withdraw: unstaking amount isn't eligible for reward", async function () {
      ...
      await expect(pd1.connect(user2).stake({ value: toPeb(1500n) })).to.emit(pd1, "Staked");
      ...
      await expect(pd1.connect(user1).cancelApprovedStakingWithdrawal(0)).to.emit(pd1, "RequestCancelWithdrawal");

      // Reward for user1 = 500 * 1000 / 2000 = 250 KAIA.
      expect(await pd1.previewRedeem(await pd1.maxRedeem(user1.address))).to.be.closeTo(toPeb(1250n), DIRTY);

      // Reward for user2 = 1500 * 1000 / 2000 = 750 KAIA.
      expect(await pd1.previewRedeem(await pd1.maxRedeem(user2.address))).to.be.closeTo(toPeb(2250n), DIRTY);
    });
    ...
    it("#cancelApprovedStakingWithdrawal: check the shares are restored correctly", async function () {
      ...
      expect(await pd1.maxWithdraw(user1.address)).to.be.closeTo(parseEther("2333.3333"), DIRTY);
    ...
    it("#cancelApprovedStakingWithdrawal: user can't cancel empty approved withdrawal", async function () {
      ...
      await expect(pd1.connect(user1).cancelApprovedStakingWithdrawal(1)).to.be.revertedWith(
    ...
    it("#cancelApprovedStakingWithdrawal: user can't cancel other users withdrawal", async function () {
      ...
      await expect(pd1.connect(user2).cancelApprovedStakingWithdrawal(0)).to.be.revertedWith(
        "Not the owner of the request.",
      );
    });
    it("#cancelApprovedStakingWithdrawal: user can't cancel already withdrawn request", async function () {
      ...
      await expect(pd1.connect(user1).cancelApprovedStakingWithdrawal(0)).to.be.revertedWith("Invalid state.");
    });
    it("#claim: user can withdraw after the lockup period", async function () {
      ...
      expect(await ethers.provider.getBalance(cnV3s[0].target)).to.be.eq(toPeb(0n));
      expect(await pd1.balanceOf(user1.address)).to.be.eq(0);
      expect(await pd1.maxRedeem(user1.address)).to.be.eq(0);
      expect(await pd1.totalAssets()).to.be.eq(toPeb(0n));
      expect(await pd1.totalSupply()).to.be.eq(toPeb(0n));
    ...
    it("#claim: user can't claim other users claimable withdrawal", async function () {
      ...
      await expect(pd1.connect(user2).claim(0)).to.be.revertedWith("Not the owner of the request.");
    });
  ...
  describe("Others about commission and fee", function () {
    it("#others: send commission to the commissionTo", async function () {
      const { pd1, user1, commissionTo } = await loadFixture(cnV3PublicDelegationFixture);
      ...
      // Manual sweep.
      await expect(pd1.sweep()).to.be.emit(pd1, "SendCommission").withArgs(commissionTo[0], toPeb(100n));
  ...
  describe("Simple redelegation tests", function () {
    it("#redelegateByShares (redelegateByAssets): user can redelegate staked amount", async function () {
      const { cnV3s, pd1, pd2, user1 } = await loadFixture(cnV3PublicDelegationFixture);
      ...
      await expect(pd1.connect(user1).stake({ value: toPeb(1000n) })).to.emit(pd1, "Staked");
      // User1 redelegates 1000 KAIA to cnV3s[1].
      await expect(pd1.connect(user1).redelegateByShares(cnV3s[1].target, toPeb(1000n)))
        .to.emit(cnV3s[0], "Redelegation")
        .withArgs(user1.address, cnV3s[1].target, toPeb(1000n))
        .to.emit(pd1, "Redelegated")
        .withArgs(user1.address, cnV3s[1].target, toPeb(1000n))
        .to.emit(cnV3s[1], "DelegateKaia")
        .to.emit(cnV3s[1], "HandleRedelegation")
        .withArgs(user1.address, cnV3s[0].target, cnV3s[1].target, toPeb(1000n))
        .to.emit(pd2, "Staked");
      ...
      expect(await pd1.maxRedeem(user1.address)).to.be.eq(toPeb(0n));
      // pd2:
      expect(await ethers.provider.getBalance(cnV3s[1].target)).to.be.eq(toPeb(1000n));
    ...
    it("#redelegateByShares (redelegateByAssets): shares must be calculated correctly", async function () {
      const { cnV3s, pd1, pd2, user1, user2 } = await loadFixture(cnV3PublicDelegationFixture);
      ...
      await setBalance(await pd1.getAddress(), parseEther("500"));

      // User1 redelegates 500 shares to cnV3s[1] => 583.3333 KAIA.
      await pd1.connect(user1).redelegateByShares(cnV3s[1].target, toPeb(500n));

      // Check the shares are calculated correctly.
      // User1 has 583.3333 KAIA in pd1 and pd2.
      expect(await pd1.maxWithdraw(user1.address)).to.be.closeTo(parseEther("583.3333"), DIRTY);
      expect(await pd2.maxWithdraw(user1.address)).to.be.closeTo(parseEther("583.3333"), DIRTY);

      // User2 redelegates 1000 KAIA to cnV3s[1] => 1166.6666 KAIA.
      await pd1.connect(user2).redelegateByAssets(cnV3s[1].target, toPeb(1000n));

      // Check the shares are calculated correctly.
      expect(await pd1.maxWithdraw(user2.address)).to.be.closeTo(parseEther("1333.33336"), DIRTY);
      expect(await pd2.maxWithdraw(user2.address)).to.be.closeTo(parseEther("1000"), DIRTY);
    });
    it("#redelegateByShares (redelegateByAssets): user can't redelegate if isRedelegationEnabled is false", async function () {
      ...
      await expect(pd1.connect(user1).stake({ value: toPeb(1000n) })).to.emit(pd1, "Staked");
      await expect(pd1.connect(user1).redelegateByShares(cnV3s[1].target, toPeb(1000n))).to.be.revertedWith(
        "Redelegation disabled.",
      );
      await expect(pd1.connect(user1).redelegateByAssets(cnV3s[1].target, toPeb(1000n))).to.be.revertedWith(
        "Redelegation disabled.",
      );
    });
    ...
    it("#redelegateByShares (redelegateByAssets): user can't redelegate more than staked amount", async function () {
      ...
      await expect(pd1.connect(user1).stake({ value: toPeb(1000n) })).to.emit(pd1, "Staked");
      await expect(pd1.connect(user1).redelegateByShares(cnV3s[1].target, toPeb(1001n))).to.be.revertedWithCustomError(
        pd1,
        "ERC20InsufficientBalance",
      );
      await expect(pd1.connect(user1).redelegateByAssets(cnV3s[1].target, toPeb(1001n))).to.be.revertedWithCustomError(
        pd1,
        "ERC20InsufficientBalance",
      );
    });
    it("#redelegateByShares (redelegateByAssets): user can't redelegate to same CnStakingV3", async function () {
      ...
      await expect(pd1.connect(user1).stake({ value: toPeb(1000n) })).to.emit(pd1, "Staked");
      await expect(pd1.connect(user1).redelegateByShares(cnV3s[0].target, toPeb(1000n))).to.be.revertedWith(
        "Target can't be self.",
      );
      await expect(pd1.connect(user1).redelegateByAssets(cnV3s[0].target, toPeb(1000n))).to.be.revertedWith(
        "Target can't be self.",
      );
    });
    it("#redelegateByShares (redelegateByAssets): user can't redelegate consecutive times", async function () {
      ...
      await addTime(WEEK / 2);
      await expect(pd2.connect(user1).redelegateByShares(cnV3s[0].target, toPeb(1000n))).to.be.revertedWith(
        "Can't redelegate yet.",
      );
      await expect(pd2.connect(user1).redelegateByAssets(cnV3s[0].target, toPeb(1000n))).to.be.revertedWith(
        "Can't redelegate yet.",
      );
      ...
      await expect(pd2.connect(user1).redelegateByShares(cnV3s[0].target, toPeb(1000n)))
        .to.emit(cnV3s[1], "Redelegation")
        .to.emit(pd2, "Redelegated")
        .withArgs(user1.address, cnV3s[0].target, toPeb(1000n))
        .to.emit(cnV3s[0], "DelegateKaia")
        .to.emit(cnV3s[0], "HandleRedelegation")
        .withArgs(user1.address, cnV3s[1].target, cnV3s[0].target, toPeb(1000n))
        .to.emit(pd1, "Staked");
      ...
      expect(await pd1.maxRedeem(user1.address)).to.be.eq(toPeb(1000n));
      // pd2:
      expect(await ethers.provider.getBalance(cnV3s[1].target)).to.be.eq(toPeb(0n));
    ...
    it("#redelegateByShares (redelegateByAssets): user can't redelegate to wrong CnStakingV3", async function () {
      const { pd1, cnV2, user1, user2 } = await loadFixture(cnV3PublicDelegationFixture);

      await expect(pd1.connect(user1).stake({ value: toPeb(1000n) })).to.emit(pd1, "Staked");

      await expect(pd1.connect(user1).redelegateByShares(user2.address, toPeb(1000n))).to.be.reverted;
      await expect(pd1.connect(user1).redelegateByShares(cnV2.target, toPeb(1000n))).to.be.revertedWith(
        "Invalid CnStakingV3.",
      );
      await expect(pd1.connect(user1).redelegateByAssets(cnV2.target, toPeb(1000n))).to.be.revertedWith(
        "Invalid CnStakingV3.",
      );
    });
    it("#redelegateByShares (redelegateByAssets): user can't redelegate to CN not registered at address book", async function () {
      const { pd1, deployer } = await loadFixture(cnV3PublicDelegationFixture);

      const fakeCnV3 = await new MockCnStakingOverV2__factory(deployer).deploy();

      await fakeCnV3.mockSetVersion(3);
      await fakeCnV3.mockSetNodeId(deployer.address);

      await expect(pd1.redelegateByShares(fakeCnV3.target, toPeb(1000n))).to.be.revertedWith("Invalid CN node ID.");
      await expect(pd1.redelegateByAssets(fakeCnV3.target, toPeb(1000n))).to.be.revertedWith("Invalid CN node ID.");
    });
  });
  describe("Simple reward tests", function () {
    it("Reward successfully captured", async function () {
      ...
      // PD captures 1000 KAIA.
      await setBalance(await pd1.getAddress(), BigInt(toPeb(1000n)));
    ...
    it("Reward automatically compounded", async function () {
      ...
      await expect(pd1.connect(user2).stake({ value: toPeb(1000n) })).to.emit(pd1, "Staked");

      // User2's share = 1000 * 1000 / 2000 = 500 * 10^18.
      expect(await ethers.provider.getBalance(pd1.target)).to.be.eq(toPeb(0n));
  ...
  describe("Scenario tests", function () {
    ...
    async function assetsForShares(pd: PublicDelegation, shares: string) {
      return await pd.convertToAssets(shares);
    }
    async function checkPSStates(pd: any, users: any[], expectedShares: any[]) {
      for (let i = 0; i < users.length; i++) {
        expect(await pd.maxRedeem(users[i].address)).to.be.closeTo(expectedShares[i], DIRTY);
      }

      let totalAssets = 0n;
      let totalSupply = 0n;
      for (let i = 0; i < expectedShares.length; i++) {
        totalAssets += await pd.convertToAssets(expectedShares[i]);
    ...
    this.beforeEach(async function () {
      ...
      {
        await expect(pd1.connect(user1).stake({ value: toPeb(1000n) })).to.emit(pd1, "Staked");
        await setBalance(await pd1.getAddress(), parseEther("6.4"));
      }

      // User2 stakes for 1500 shares.
      {
        await expect(pd1.connect(user2).stake({ value: await assetsForShares(pd1, toPeb(1500n)) })).to.emit(
          pd1,
          "Staked",
        );
        await setBalance(await pd1.getAddress(), parseEther("6.4"));
      ...
      {
        await expect(pd1.connect(user3).stake({ value: await assetsForShares(pd1, toPeb(500n)) })).to.emit(
          pd1,
          "Staked",
        );
        await setBalance(await pd1.getAddress(), parseEther("6.4"));
    ...
    it("3 users stake and withdraw", async function () {
      ...
      // Check the states.
      await checkPSStates(pd1, [user1, user2, user3], [toPeb(1000n), toPeb(1500n), toPeb(500n)]);
      ...
      // User1 withdraws 1000 shares.
      {
        ...
        expect((await cnV3s[0].staking()) - (await cnV3s[0].unstaking())).to.be.eq(0n);
      }
      ...
      {
        await expect(pd1.connect(user1).claim(2)).to.emit(cnV3s[0], "WithdrawApprovedStaking").to.emit(pd1, "Claimed");
        await expect(pd1.connect(user2).claim(1)).to.emit(cnV3s[0], "WithdrawApprovedStaking").to.emit(pd1, "Claimed");
        await expect(pd1.connect(user3).claim(0)).to.emit(cnV3s[0], "WithdrawApprovedStaking").to.emit(pd1, "Claimed");
      ...
      expect(await ethers.provider.getBalance(commissionTo[0])).to.be.closeTo(
        balanceCommissionTo1 + parseEther("3.2"),
        DIRTY,
      );

      // User1's balance is increased by approx. (6.4 * 2 + 2.56 * 2 + 2.13333) * 0.9 = 18.048 KAIA.
      expect(await ethers.provider.getBalance(user1.address)).to.be.closeTo(balanceA + parseEther("18.048"), DIRTY);

      // User2's balance is increased by approx. (3.84 * 2 + 3.2) * 0.9 = 9.792 KAIA.
      expect(await ethers.provider.getBalance(user2.address)).to.be.closeTo(balanceB + parseEther("9.792"), DIRTY);
      ...
      expect(await ethers.provider.getBalance(user3.address)).to.be.closeTo(
        balanceC + parseEther("0.959999994"),
        DIRTY,
      );
      ...
      expect(await ethers.provider.getBalance(await pd1.getAddress())).to.be.eq(toPeb(0n));
      expect(await pd1.totalAssets()).to.be.eq(toPeb(0n));
      expect(await pd1.totalSupply()).to.be.eq(toPeb(0n));
    });
    it("3 users stake and redelegate", async function () {
      ...
      {
        await pd1.connect(user1).redelegateByAssets(cnV3s[1].target, toPeb(1000n));
        // A's shares in PD1: 9.885305113744376148
        // A's shares in PD2: 1000
        await setBalance(await pd1.getAddress(), parseEther("6.4"));
        await setBalance(await pd2.getAddress(), parseEther("6.4"));

        await checkPSStates(
          pd1,
          [user1, user2, user3],
          [await pd1.maxRedeem(user1.address), toPeb(1500n), toPeb(500n)],
        );
      ...
      {
        await pd1.connect(user2).redelegateByAssets(cnV3s[1].target, await assetsForShares(pd2, toPeb(500n)));
        ...
        await checkPSStates(
          pd1,
          [user1, user2, user3],
          [await pd1.maxRedeem(user1.address), await pd1.maxRedeem(user2.address), toPeb(500n)],
        );
        await checkPSStates(pd2, [user1, user2, user3], [toPeb(1000n), toPeb(500n), 0n]);
      }

      // User3 withdraws all shares.
      {
        await pd1.connect(user3).redeem(user3.address, await pd1.maxRedeem(user3.address));
        await setBalance(await pd1.getAddress(), parseEther("6.4"));
        await checkPSStates(
          pd1,
          [user1, user2, user3],
          [await pd1.maxRedeem(user1.address), await pd1.maxRedeem(user2.address), 0n],
        );
        await checkPSStates(pd2, [user1, user2, user3], [toPeb(1000n), toPeb(500n), 0n]);
      }
      ...
      {
        await pd1.connect(user2).redeem(user2.address, await pd1.maxRedeem(user2.address));
        await pd2.connect(user2).redeem(user2.address, await pd2.maxRedeem(user2.address));

        await setBalance(await pd1.getAddress(), parseEther("6.4"));
        /**
         * @dev Adds 12.8 KAIA since the previous `setBalance` will be overwritten by the next `setBalance`.
         */
        await setBalance(await pd2.getAddress(), parseEther("12.8"));

        await checkPSStates(pd1, [user1, user2, user3], [await pd1.maxRedeem(user1.address), 0n, 0n]);
        await checkPSStates(pd2, [user1, user2, user3], [toPeb(1000n), 0n, 0n]);
      }

      // User1 withdraws all shares.
      {
        await pd1.connect(user1).redeem(user1.address, await pd1.maxRedeem(user1.address));
        await pd2.connect(user1).redeem(user1.address, await pd2.maxRedeem(user1.address));
      ...
      {
        await expect(pd1.connect(user1).claim(2)).to.emit(cnV3s[0], "WithdrawApprovedStaking").to.emit(pd1, "Claimed");
        await expect(pd2.connect(user1).claim(1)).to.emit(cnV3s[1], "WithdrawApprovedStaking").to.emit(pd2, "Claimed");

        await expect(pd1.connect(user2).claim(1)).to.emit(cnV3s[0], "WithdrawApprovedStaking").to.emit(pd1, "Claimed");
        await expect(pd2.connect(user2).claim(0)).to.emit(cnV3s[1], "WithdrawApprovedStaking").to.emit(pd2, "Claimed");
      ...
      const commission = await ethers.provider.getBalance(commissionTo[0]);
      expect(commission).to.be.eq(balanceCommissionTo1 + parseEther("4.48"));

      const commission2 = await ethers.provider.getBalance(commissionTo[1]);
      expect(commission2).to.be.eq(balanceCommissionTo2 + parseEther("2.56"));
      ...
      expect(await ethers.provider.getBalance(user1.address)).to.be.closeTo(
        balanceA + parseEther("36.98614076"),
        DIRTY * 2n,
      );
      ...
      expect(await ethers.provider.getBalance(user2.address)).to.be.closeTo(
        balanceB + parseEther("22.07792324"),
        DIRTY * 2n,
      );

      // User3's balance is rewarded by approx. 4.77326222 * 0.9 = 4.29593600 KAIA
      expect(await ethers.provider.getBalance(user3.address)).to.be.closeTo(
        balanceC + parseEther("4.29593600"),
        DIRTY * 2n,
      );

      // Check CnV3, PD states.
      for (let i = 0; i < cnV3s.length; i++) {
        expect(await ethers.provider.getBalance(cnV3s[i].target)).to.be.eq(toPeb(0n));
        expect(await cnV3s[i].staking()).to.be.eq(toPeb(0n));
        expect(await cnV3s[i].unstaking()).to.be.eq(toPeb(0n));
      }
      expect(await ethers.provider.getBalance(pd1.target)).to.be.eq(toPeb(0n));
      expect(await ethers.provider.getBalance(pd2.target)).to.be.eq(toPeb(0n));
      expect(await pd1.totalAssets()).to.be.eq(toPeb(0n));
      expect(await pd1.totalSupply()).to.be.eq(toPeb(0n));
      expect(await pd2.totalAssets()).to.be.eq(toPeb(0n));
      expect(await pd2.totalSupply()).to.be.eq(toPeb(0n));
  ...
  describe("Getter tests", function () {
    ...
    it("Withdrawal requests", async function () {
      ...
      expect(await pd1.getUserRequestIds(user2.address)).to.equalNumberList([1]);
      expect(await pd1.getUserRequestIds(user3.address)).to.equalNumberList([0]);
    ...
    it("Withdrawal requests with given states", async function () {
      const { pd1, user1, user2, user3 } = fixture;
      expect(await pd1.getUserRequestIdsWithState(user1.address, 0)).to.equalNumberList([]);
      expect(await pd1.getUserRequestIdsWithState(user2.address, 1)).to.equalNumberList([1]);
      expect(await pd1.getUserRequestIdsWithState(user3.address, 3)).to.equalNumberList([0]);
    });
    it("Withdrawal states", async function () {
      ...
      expect(await cnV3s[0].getApprovedStakingWithdrawalIds(0, 0, 1)).to.be.equalNumberList([0]);
      // If _to > withdrawalRequestCount, then _to = withdrawalRequestCount.
      expect(await cnV3s[0].getApprovedStakingWithdrawalIds(0, 10, 2)).to.be.equalNumberList([1]);
    });
    it("Account states", async function () {
      const { pd1, user1, user2, user3 } = fixture;
      expect(await pd1.balanceOf(user1.address)).to.equal(parseEther("1000"));
      expect(await pd1.balanceOf(user2.address)).to.equal(parseEther("806.321041427723505759"));
      expect(await pd1.balanceOf(user3.address)).to.equal(parseEther("141.529474243228890069"));

      expect(await pd1.maxWithdraw(user1.address)).to.equal(parseEther("1009.112344189786891837"));
      expect(await pd1.maxWithdraw(user2.address)).to.equal(parseEther("813.668516284680337756"));
      expect(await pd1.maxWithdraw(user3.address)).to.equal(parseEther("142.819139525532770406"));
    });
    it("Precision test for staking", async function () {
      ...
      expect(await pd1.previewDeposit(parseEther("1.009112344189786891"))).to.equal(parseEther("0.999999999999999999"));
      expect(await pd1.previewDeposit(parseEther("1.009112344189786892"))).to.equal(parseEther("1"));
    });
    it("Precision test for withdrawal", async function () {
      const { pd1 } = fixture;
      // Currently, 1 pKLAY = 1.009112344189786891(8) KAIA
      // User will receive floored amount of KAIA.
      expect(await pd1.previewRedeem(parseEther("0.999999999999999999"))).to.equal(parseEther("1.009112344189786890"));
      expect(await pd1.previewRedeem(parseEther("1"))).to.equal(parseEther("1.009112344189786891"));
    });
  });
});
